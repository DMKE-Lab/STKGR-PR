...
def beam_search(pn, e_s, q, tim, e_t, kg, num_steps, beam_size, return_path_components=False, use_kg_pred=False, kg_pred=None, fn_kg=None):
    """
    ...
    def top_k_action(log_action_dist, action_space):
        ...
        (r_space, t_space, e_space), _ = action_space
        # 改
        action_space_size = r_space.size()[1] + t_space.size()[1]
        ...
        next_t = ops.batch_lookup(t_space.view(batch_size, -1), action_ind).view(-1)
        ...
        return (next_r, next_t, next_e), log_action_prob, action_offset
    ...
    def top_k_answer_unique(log_action_dist, action_space):
        ...
        for i in range(batch_size):
            ...
            top_t = t_space_b[top_unique_idx]
            top_e = e_space_b[top_unique_idx]
            next_r_list.append(top_r.unsqueeze(0))
            next_t_list.append(top_t.unsqueeze(0))
        ...
        next_t = ops.pad_and_cat(next_t_list, padding_value=kg.dummy_t).view(-1)
        ...
        return (next_r, next_t, next_e), log_action_prob.view(-1), action_offset.view(-1)
    
    def adjust_search_trace(search_trace, action_offset):
        for i, (r, t, e) in enumerate(search_trace):
            new_r = r[action_offset]
            new_t = t[action_offset]
            new_e = e[action_offset]
            search_trace[i] = (new_r, new_t, new_e)
    ...
    init_action = (r_s, t_s, e_s)
    # path encoder
    pn.initialize_path(init_action, kg)

    # # 改
    kg.args.save_paths_to_csv = True

    if kg.args.save_paths_to_csv:
        search_trace = [(r_s, t_s, e_s)]
    ...
    for t in range(num_steps):
        last_r, last_t, e = action
        assert(q.size() == e_s.size())
        assert(tim.size() == e_s.size())
        assert(e.size()[0] % batch_size == 0)
        assert(q.size()[0] % batch_size == 0)
        assert(tim.size()[0] % batch_size == 0)
        k = int(e.size()[0] / batch_size)
        # => [batch_size*k]
        q = ops.tile_along_beam(q.view(batch_size, -1)[:, 0], k)
        tim = ops.tile_along_beam(tim.view(batch_size, -1)[:, 0], k)
        e_s = ops.tile_along_beam(e_s.view(batch_size, -1)[:, 0], k)
        if use_kg_pred:
            kg_pred_tmp = ops.tile_along_beam(kg_pred.view(batch_size, -1), k)
        # e_t_tmp = ops.tile_along_beam(e_t.view(batch_size, -1), k)
        ...
        if use_kg_pred:
            ...
            e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=True, kg_pred=kg_pred_tmp, fn_kg=fn_kg, use_kg_pred=True, inference=True)
        else:
            db_outcomes, _, _ = pn.transit(
            e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=True, inference=True)
        ...
        seen_nodes = torch.cat([seen_nodes[action_offset], action[2].unsqueeze(1)], dim=1)
        if kg.args.save_paths_to_csv:
            adjust_search_trace(search_trace, action_offset)
            search_trace.append(action)

    output_beam_size = int((action[0].size()[0] + action[1].size()[0]) / batch_size)
    # [batch_size*beam_size] => [batch_size, beam_size]
    beam_search_output = dict()
    beam_search_output['pred_e2s'] = action[2].view(batch_size, -1)
        ...
        for i in range(batch_size):
            ...
            for k, log_action_prob in enumerate(log_action_probs):
                ...
                for j in range(min(output_beam_size, path_width)):
                    ...
                    t = kg.id2time[int(search_trace[k+1][1][ind])]
                    e = kg.id2entity[int(search_trace[k+1][2][ind])]
                    if r.endswith('_inv'):
                        edge_label = '<-{}-{}-{} {}'.format(r[:-4], t[:-4], e, float(log_action_probs[k][ind]))
                    else:
                        edge_label = '-{}{}->{} {}'.format(r, t, e, float(log_action_probs[k][ind]))
