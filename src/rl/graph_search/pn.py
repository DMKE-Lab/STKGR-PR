...
class GraphSearchPolicy(nn.Module):
    def __init__(self, args, fn=None, fn_kg=None):
        ...
        if self.relation_only:
            self.action_dim = args.relation_dim + args.time_dim
        else:
            self.action_dim = args.entity_dim + args.relation_dim + args.time_dim
        ...
        self.time_only_in_path = args.time_only_in_path
        self.path = None
    ...
    def transit(self, e, obs, kg, kg_pred=None, fn_kg=None, use_action_space_bucketing=True, merge_aspace_batching_outcome=False, use_kg_pred=False, inference=False):
        ...
        :return
            ...
            Else:
                ...
                entropy: (Batch) entropy of action distribution.
        """
        e_s, q, tim, e_t, last_step, last_r, last_t, seen_nodes = obs
        # print(q.size())
        # print("++++++")
        # print(tim.size())
        # Representation of the current state (current node and other observations)
        Q = kg.get_relation_embeddings(q)
        T = kg.get_time_embeddings(tim)
        H = self.path[-1][0][-1, :, :]
        # print(H)
        # print(H.size())
        if self.relation_only and self.time_only:
            X = torch.cat([H, Q, T], dim=-1)
        elif self.relation_only_in_path and self.time_only_in_path:
            E_s = kg.get_entity_embeddings(e_s)
            E = kg.get_entity_embeddings(e)
            X = torch.cat([E, H, E_s, Q, T], dim=-1)
        elif use_kg_pred:
            E = kg.get_entity_embeddings(e)
            X = torch.cat([E, H, Q, T, kg_pred], dim=-1)
            # X = torch.cat([E, H, Q, kg.get_entity_embeddings(e_t)], dim=-1)
        else:
            E = kg.get_entity_embeddings(e)
            X = torch.cat([E, H, Q, T], dim=-1)
        ...
        time_att = torch.matmul(self.W_att(X2), kg.get_all_time_embeddings().t())
        # B x |R|
        # Trick -> mask SIM relation
        relation_att = torch.nn.functional.softmax(relation_att, dim=-1)
        time_att = torch.nn.functional.softmax(time_att, dim=-1)

        def policy_nn_fun(X2, action_space):
            # print("*********")
            (r_space, t_space, e_space), action_mask = action_space
            A = self.get_action_embedding((r_space, t_space, e_space), kg)
        ...
        def pad_and_cat_action_space(action_spaces, inv_offset):
            db_r_space, db_t_space, db_e_space, db_action_mask = [], [], [], []
            for (r_space, t_space, e_space), action_mask in action_spaces:
                db_r_space.append(r_space)
                db_t_space.append(t_space)
            ...
            t_space = ops.pad_and_cat(db_t_space, padding_value=kg.dummy_t)[inv_offset]
            ...
            action_space = ((r_space, t_space, e_space), action_mask)
            return action_space
        ...
        if use_action_space_bucketing:
            ...
            entropy_list = []
            references = []
            db_action_spaces, db_references = self.get_action_space_in_buckets(e, obs, kg, relation_att=relation_att, time_att=time_att, inference=inference)
    ...
    def initialize_path(self, init_action, kg):
        ...
        if self.relation_only_in_path:
            init_action_embedding = torch.cat(kg.get_relation_embeddings(init_action[0]), kg.get_time_embeddings(init_action[1]))
        else:
            init_action_embedding = self.get_action_embedding(init_action, kg)
        # print(init_action_embedding.size())
    ...
    def update_path(self, action, kg, offset=None):
        ...
        if self.relation_only_in_path:
            print("aa")
            action_embedding = torch.cat([kg.get_relation_embeddings(action[0]), kg.get_time_embeddings(action[1])])
        else:
    ...
    def get_dynamic_action_space(self, e_space, r_space, t_space, action_mask, e_b, relation_att, time_att):
        ...
        # print(additional_relation_size)

        additional_time_size = int(additional_action_space_size / self.avg_entity_per_relation) + 1
        # print(additional_time_size)

        # bks x additional_relation_size
        relation_idx = torch.multinomial(relation_att, additional_relation_size)
        time_idx = torch.multinomial(time_att, additional_time_size)
        # bks x additional_relation_size x |E|
        S = self.fn.forward(e_b.repeat_interleave(additional_relation_size, dim=0), relation_idx.view(bks * additional_relation_size), time_idx.view(bks * additional_time_size), self.fn_kg).view(bks, additional_relation_size, self.fn_kg.num_entities)
                       # +self.fn.forward(e_b.repeat_interleave(additional_time_size, dim=0), relation_idx.view(bks * additional_relation_size),
                       #                 time_idx.view(bks * additional_time_size),
                       #                 self.fn_kg).view(bks, additional_relation_size, self.fn_kg.num_entities)
        ...
        new_t_space = time_idx.repeat_interleave(self.avg_entity_per_relation, dim=1)
        ...
        t_space = torch.cat([t_space, new_t_space], dim=-1)
        action_mask = torch.cat([action_mask, new_action_mask], dim=-1)
        return e_space, r_space, t_space, action_mask

    def get_action_space_in_buckets(self, e, obs, kg, collapse_entities=False, relation_att=None, time_att=None, inference=False):
        """
        ...
        assert(len(e) == len(q))
        assert(len(e) == len(tim))
        assert(len(e) == len(last_t))
        ...
        if collapse_entities:
            ...
            for key in batch_ref:
                ...
                t_space_b = action_space[0][1][g_bucket_ids]
                e_space_b = action_space[0][2][g_bucket_ids]
                action_mask_b = action_space[1][g_bucket_ids]
                e_b = e[l_batch_refs]
                if inference or random.random() < 0.5:
                    if self.use_action_selection:
                        e_space_b, r_space_b, t_space_b, action_mask_b = self.get_dynamic_action_space(e_space_b, r_space_b, t_space_b, action_mask_b, e_b, relation_att[l_batch_refs], time_att[l_batch_refs])

                last_r_b = last_r[l_batch_refs]
                last_t_b = last_t[l_batch_refs]
                e_s_b = e_s[l_batch_refs]
                q_b = q[l_batch_refs]
                t_b = tim[l_batch_refs]
                e_t_b = e_t[l_batch_refs]
                seen_nodes_b = seen_nodes[l_batch_refs]
                obs_b = [e_s_b, q_b, t_b, e_t_b, last_step, last_r_b, last_t_b, seen_nodes_b]
                action_space_b = ((r_space_b, t_space_b, e_space_b), action_mask_b)
        ...
        # print(len(db_action_spaces[0]))
        return db_action_spaces, db_references

    def get_action_space(self, e, obs, kg):
        r_space, t_space, e_space = kg.action_space[0][0][e], kg.action_space[0][1][e], kg.action_space[0][2][e]
        print("######")
    ...
    def apply_action_masks(self, action_space, e, obs, kg):
        ...
        if len(e_t.shape) == 1 or e_t.shape[-1] == 1:
            ground_truth_edge_mask = self.get_ground_truth_edge_mask(e, r_space, t_space, e_space, e_s, q, tim, e_t, kg)
            action_mask -= ground_truth_edge_mask
            self.validate_action_mask(action_mask)
        else:
            ground_truth_edge_mask = self.get_ground_truth_edge_mask_multi(e, r_space, t_space, e_space, e_s, q, tim, e_t, kg)
            action_mask -= ground_truth_edge_mask
            self.validate_action_mask(action_mask)
        ...
        return (r_space, t_space, e_space), action_mask

    def get_ground_truth_edge_mask(self, e, r_space, t_space, e_space, e_s, q, tim, e_t, kg):
        ground_truth_edge_mask = \
            ((e == e_s).unsqueeze(1) * (r_space == q.unsqueeze(1)) * (t_space == tim.unsqueeze(1)) * (e_space == e_t.unsqueeze(1)))
        inv_q = kg.get_inv_relation_id(q)
        inv_t = kg.get_inv_time_id(tim)
        inv_ground_truth_edge_mask = \
            ((e == e_t).unsqueeze(1) * (r_space == inv_q.unsqueeze(1)) * (t_space == inv_t.unsqueeze(1)) * (e_space == e_s.unsqueeze(1)))
        return ((ground_truth_edge_mask + inv_ground_truth_edge_mask) * (e_s.unsqueeze(1) != kg.dummy_e)).float()

    def get_ground_truth_edge_mask_multi(self, e, r_space, t_space, e_space, e_s, q, tim, e_t, kg):
        ans_1 = (torch.gather(e_t, 1, e_space) == 1)
        ans_2 = (torch.gather(e_t, 1, e.unsqueeze(-1)) == 1)

        ground_truth_edge_mask = \
            ((e == e_s).unsqueeze(1) * (r_space == q.unsqueeze(1)) * (t_space == tim.unsqueeze(1)) * ans_1)
        inv_q = kg.get_inv_relation_id(q)
        inv_t = kg.get_inv_time_id(tim)
        inv_ground_truth_edge_mask = \
            (ans_2 * (r_space == inv_q.unsqueeze(1)) * (t_space == inv_t.unsqueeze(1)) * (e_space == e_s.unsqueeze(1)))
        return ((ground_truth_edge_mask + inv_ground_truth_edge_mask) * (e_s.unsqueeze(1) != kg.dummy_e)).float()
    ...
    def get_action_embedding(self, action, kg):
        ...
        time_embedding = kg.get_time_embeddings(t)
        if self.relation_only:
            action_embedding = torch.cat(relation_embedding, time_embedding)
        else:
            entity_embedding = kg.get_entity_embeddings(e)
            action_embedding = torch.cat([relation_embedding, time_embedding, entity_embedding], dim=-1)
        return action_embedding

    def define_modules(self):
        # if self.relation_only:
        #     input_dim = self.history_dim + torch.multiply(self.relation_dim, self.time_dim)
        # elif self.relation_only_in_path:
        #     input_dim = self.history_dim + self.entity_dim * 2 + torch.multiply(self.relation_dim, self.time_dim)
        # elif self.use_kg_pred:
        #     input_dim = self.history_dim + self.entity_dim * 2 + torch.multiply(self.relation_dim, self.time_dim)
        # else:
        #     input_dim = self.history_dim + self.entity_dim + torch.multiply(self.relation_dim, self.time_dim)
        if self.relation_only:
            input_dim = self.history_dim + self.relation_dim + self.time_dim
        elif self.relation_only_in_path:
            input_dim = self.history_dim + self.entity_dim * 2 + self.relation_dim + self.time_dim
        elif self.use_kg_pred:
            input_dim = self.history_dim + self.entity_dim * 2 + self.relation_dim + self.time_dim
        else:
            input_dim = self.history_dim + self.entity_dim + self.relation_dim + self.time_dim
        ...
        if self.relation_only_in_path:
            self.path_encoder = nn.LSTM(input_size=self.relation_dim + self.time_dim,
