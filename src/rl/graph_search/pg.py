...
class PolicyGradient(LFramework):
    ...
    def loss(self, mini_batch):
        ...
        e1, e2, r, t = self.format_batch(mini_batch, num_tiles=self.num_rollouts)
        output = self.rollout(e1, r, t, e2, num_steps=self.num_rollout_steps)
    ...
    def rollout(self, e_s, q, tim, e_t, num_steps, visualize_action_probs=False):
        """
    ...
    def sample_action(self, db_outcomes, inv_offset=None):
        ...
        def apply_action_dropout_mask(action_dist, action_mask):
            if self.action_dropout_rate > 0:
                ...
                # action_keep_mask_np = action_keep_mask.cpu().detach().numpy()
                # np.save('action_keep_mask.npy', action_keep_mask_np)
                # action_mask_np = action_mask.cpu().detach().numpy()
                # np.save('action_mask.npy', action_mask_np)
        ...
        def sample(action_space, action_dist):
            ...
            # M_np = r_space.cpu().detach().numpy()
            # np.save('M.npy', M_np)
            # this is fine

            # action_dist_np = action_dist.cpu().detach().numpy()
            # np.save('action_dist.npy', action_dist_np)

            sample_action_dist = apply_action_dropout_mask(action_dist, action_mask)
            # sample_action_dist_np = sample_action_dist.cpu().detach().numpy()
            # np.save('sample_action_dist.npy', sample_action_dist_np)

            idx = torch.multinomial(sample_action_dist, 1, replacement=True)
            # idx wrong
            next_r = ops.batch_lookup(r_space, idx)
            next_t = ops.batch_lookup(t_space, idx)
            next_e = ops.batch_lookup(e_space, idx)
            action_prob = ops.batch_lookup(action_dist, idx)
            sample_outcome['action_sample'] = (next_r, next_t, next_e)
        ...
        if inv_offset is not None:
            ...
            for action_space, action_dist in db_outcomes:
                ...
                next_t_list.append(sample_outcome['action_sample'][1])
                next_e_list.append(sample_outcome['action_sample'][2])
            ...
            next_t = torch.cat(next_t_list, dim=0)[inv_offset]
            next_e = torch.cat(next_e_list, dim=0)[inv_offset]
            action_sample = (next_r, next_t, next_e)
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        beam_search_output = search.beam_search(
            pn, e1, r, t, e2, kg, self.num_rollout_steps, self.beam_size)
