...
from src.emb.fact_network import get_conve_nn_state_dict, get_conve_kg_state_dict, \
    ...
    get_tucker_nn_state_dict, get_tucker_kg_state_dict, get_TransE_kg_state_dict
...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        if fn_model in ['conve']:
            ...
            fn_state_dict = torch.load(args.TransE_state_dict_path, map_location=('cuda:' + str(args.gpu)))
    ...
    def reward_fun(self, e1, r, t, e2, pred_e2, path_trace):
        if self.model.endswith('.rso'):
            oracle_reward = forward_fact_oracle(e1, r, t, pred_e2, self.kg)
            return oracle_reward
        else:
            if self.fn_secondary_kg:
                real_reward = self.fn.forward_fact(e1, r, pred_e2, t, self.fn_kg, [self.fn_secondary_kg]).squeeze(1)
            elif self.fn_model == 'PTransE':
                real_reward = self.fn.forward_fact(e1, r, pred_e2, t, self.fn_kg, path_trace).squeeze(1)
            else:
                real_reward = self.fn.forward_fact(e1, r, pred_e2, t, self.fn_kg).squeeze(1)
            real_reward_mask = (real_reward > self.reward_shaping_threshold).float()
            real_reward *= real_reward_mask
            # real_reward是嵌入模型提供的软奖励
            if self.model.endswith('rsc'):
                ...
                par = 0.4
                path_reward = self.forward_path(e1, r, pred_e2, t, self.fn_kg, path_trace).squeeze(1)
                return binary_reward + self.mu * (1 - binary_reward) * (par * real_reward + (1 - par) * path_reward)

    def forward_path(self, e1, r, e2, t, kg, path_trace):
        E1 = kg.get_entity_embeddings(e1)
        R = kg.get_relation_embeddings(r)
        E2 = kg.get_entity_embeddings(e2)
        T = kg.get_time_embeddings(t)
        first = torch.abs(E1 + R + T - E2)
        second = R
        secondt = T
        for i in range(1, len(path_trace)):
            second = R - kg.get_relation_embeddings(path_trace[i][0])
            secondt = T - kg.get_time_embeddings(path_trace[i][1])
        S = torch.sum(first + torch.abs(second) + torch.abs(secondt), dim=1, keepdim=True)
    ...
    def loss(self, mini_batch):
        ...
        e1, e2, r, t, kg_pred = self.format_batch(mini_batch, num_tiles=self.num_rollouts, num_labels=self.kg.num_entities)
        output = self.rollout(e1, r, t, e2, num_steps=self.num_rollout_steps, kg_pred=kg_pred)
        ...
        final_reward = self.reward_fun(e1, r, t, e2, pred_e2, path_trace)
        ...
        if self.run_analysis:
            ...
            for i in range(len(final_reward)):
                if not final_reward[i]:
                    print(pred_e2[i])
                    print(self.kg.all_objects[int(e1[i])][int(r[i])][int(t[i])])
                    if int(pred_e2[i]) in self.kg.all_objects[int(e1[i])][int(r[i])][int(t[i])]:
                        fn[i] = 1
            loss_dict['fn'] = fn
    ...
    def format_batch(self, batch_data, num_labels=-1, num_tiles=1, inference=False):
        ...
        batch_e1, batch_e2, batch_r, batch_t = [], [], [], []
        for i in range(len(batch_data)):
            e1, e2, r, t = batch_data[i]
            batch_e1.append(e1)
            batch_e2.append(e2)
            batch_r.append(r)
            batch_t.append(t)
        if inference:
            ...
            for i in range(len(batch_e1)):
                tmp = []
                if batch_e1[i] in self.kg.train_objects and batch_r[i] in self.kg.train_objects[batch_e1[i]] and batch_t[i] in self.kg.train_objects[batch_e1[i]][batch_r[i]]:
                    tmp += list(self.kg.train_objects[batch_e1[i]][batch_r[i]][batch_t[i]])
        ...
        batch_t = var_cuda(torch.LongTensor(batch_t), requires_grad=False)
        ...
        if self.strategy == 'sample':
            # sample emb
            S = self.fn.forward(batch_e1, batch_r, batch_t, self.fn_kg).view(-1, self.kg.num_entities)
            ...
            # Top K method
            S = self.fn.forward(batch_e1, batch_r, batch_t, self.fn_kg)
            # print(S)
        ...
        if num_tiles > 1:
            ...
            batch_t = ops.tile_along_beam(batch_t, num_tiles)
            batch_e2 = ops.tile_along_beam(batch_e2, num_tiles)
            pred_kg = ops.tile_along_beam(pred_kg, num_tiles)
        return batch_e1, batch_e2, batch_r, batch_t, pred_kg

    def rollout(self, e_s, q, tim, e_t, num_steps, kg_pred, visualize_action_probs=False):
        """
        ...
        # 改
        t_s = int_fill_var_cuda(e_s.size(), kg.dummy_start_t)
        seen_nodes = int_fill_var_cuda(e_s.size(), kg.dummy_e).unsqueeze(1)
        path_components = []

        path_trace = [(r_s, t_s, e_s)]
        pn.initialize_path((r_s, t_s, e_s), kg)

        for t in range(num_steps):
            last_r, last_t, e = path_trace[-1]
            # print(tim)
            obs = [e_s, q, tim, e_t, t==(num_steps-1), last_r, last_t, seen_nodes]
    ...
    def predict(self, mini_batch, verbose=False):
        kg, pn = self.kg, self.mdl
        e1, e2, r, t, kg_pred = self.format_batch(mini_batch, inference=False)
        beam_search_output = search.beam_search(
            pn, e1, r, t, e2, kg, self.num_rollout_steps, self.beam_size, use_kg_pred=self.use_state_prediction, kg_pred=kg_pred, fn_kg=self.fn_kg)
        ...
        if verbose:
            ...
            for i in range(len(e1)):
                ...
                tim = kg.id2time[int(t[i])]
                o = kg.id2entity[int(e2[i])]
                print('({}, {}, {}, {})'.format(h, rel, tim, o))
                ...
                for j in range(output_beam_size):
                    ...
                    for k in range(len(search_traces)):
                        search_trace.append((int(search_traces[k][0][ind]), int(search_traces[k][1][ind]), int(search_traces[k][2][ind])))
                        rr = int(search_traces[k][2][ind])
                    if rr == int(e2[i]):
                        print("_____")
    ...
    def test_fn(self, examples):
        ...
        for example_id in tqdm(range(0, len(examples), self.batch_size)):
            ...
            e1, e2, r, t = self.format_batch(mini_batch)
            if self.fn_secondary_kg:
                pred_score = fn.forward_fact(e1, r, e2, t, fn_kg, [self.fn_secondary_kg])
            elif self.fn_model == 'PTransE':
                # TODO
                pred_score = fn.forward_fact(e1, r, e2, t, fn_kg)
            else:
                pred_score = fn.forward_fact(e1, r, e2, t, fn_kg)
...
def forward_fact_oracle(e1, r, t, e2, kg):
    oracle = zeros_var_cuda([len(e1), kg.num_entities]).cuda()
    for i in range(len(e1)):
        _e1, _r, _t = int(e1[i]), int(r[i]), int(t[i])
        if _e1 in kg.all_object_vectors and _r in kg.all_object_vectors[_e1] and _t in kg.all_object_vectors[_e1][_r]:
            answer_vector = kg.all_object_vectors[_e1][_r][_t]
