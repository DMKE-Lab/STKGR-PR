...
# 下面一行kg本来是None，做消融改了一下kg=KnowledgeGraph(args)=>kg=None
def hits_and_ranks(examples, scores, all_answers, verbose=False, output=False, kg=None, model_name=None, split_relation=False):
    """
    ...
    for i, example in enumerate(examples):
        # print("cc")
        e1, e2, r, t = example
        e2_multi = dummy_mask + list(all_answers[e1][r][t])
    ...
    for i in range(kg.num_relations):
        hits_at_10_list.append(0)
        example_length.append(0)
    if output:
        f = open(model_name + '_hits10_wrong.txt', 'w')
        fr = open(model_name + '_hits10_right.txt', 'w')
    for i, example in enumerate(examples):
        # print("bb")
        e1, e2, r, t = example
        example_length[r] += 1
        ...
        if len(pos) > 0:
            ...
            if pos < 10:
                if output:
                    fr.write('{}\t{}\t{}\t{}\n'.format(kg.id2entity[e1], kg.id2entity[e2], kg.id2relation[r], kg.id2time[t]))
                hits_at_10 += 1
                hits_at_10_list[r] += 1
                if pos < 5:
                    hits_at_5 += 1
    ...
    if verbose:
        ...
        if split_relation:
            split_score = {}
            for i in range(kg.num_relations):
                if example_length[i] == 0:
                    continue
                split_score[kg.id2relation[i]] = float(hits_at_10_list[i]) / example_length[i]
            l = sorted(split_score.items(), key=lambda x: x[1], reverse=True)
            for term in l:
...
def hits_at_k(examples, scores, all_answers, verbose=False):
    ...
    for i, example in enumerate(examples):
        e1, e2, r, t = example
        e2_multi = list(all_answers[e1][r][t]) + dummy_mask
        # save the relevant prediction
...
def hits_and_ranks_by_seen_queries(examples, scores, all_answers, seen_queries, verbose=False):
    ...
    _, _, _, _, seen_mrr = hits_and_ranks(seen_exps, scores[seen_ids], all_answers, verbose=False, kg=KnowledgeGraph(args))
    _, _, _, _, unseen_mrr = hits_and_ranks(unseen_exps, scores[unseen_ids], all_answers, verbose=False, kg=KnowledgeGraph(args))
    if verbose:
...
def hits_and_ranks_by_relation_type(examples, scores, all_answers, relation_by_types, verbose=False):
    ...
    _, _, _, _, to_m_mrr = hits_and_ranks(to_M_exps, scores[to_M_ids], all_answers, verbose=False, kg=KnowledgeGraph(args))
    _, _, _, _, to_1_mrr = hits_and_ranks(to_1_exps, scores[to_1_ids], all_answers, verbose=False, kg=KnowledgeGraph(args))
    if verbose:
...
def link_MAP(examples, scores, labels, all_answers, verbose=False):
    ...
    for e1 in queries:
        ranked_examples = sorted(queries[e1], key=lambda x:x[3], reverse=True)
        ...
        for i in range(len(ranked_examples)):
            ...
            if label == '+':
                ...
                answer_set = {}
                if e1 in all_answers and r in all_answers[e1] and t in all_answers[e1][r]:
                    answer_set = all_answers[e1][r][t]
