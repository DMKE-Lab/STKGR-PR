...
def format_path(path_trace, kg):
    ...
    def get_most_recent_time(j):
        time_id = int(path_trace[j][1])
        if time_id == kg.self_edge:
            return 'LOOP'
        else:
            return kg.id2time[time_id]

    def get_most_recent_entity(j):
        return kg.id2entity[int(path_trace[j][2])]

    path_str = get_most_recent_entity(0)
    for j in range(1, len(path_trace)):
        rel = get_most_recent_relation(j)
        time = get_most_recent_time(j)
        # if not rel.endswith('_inv'):
        #     path_str += '-{}->'.format(rel)
        # else:
        #     path_str += '<-{}-'.format(rel[:-4])
        path_str += '-{}-'.format(rel)
        path_str += '-{}-'.format(time)
