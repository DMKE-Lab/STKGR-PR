...
class ConvE(nn.Module):
    def __init__(self, args, num_entities):
        ...
        assert(args.emb_2D_d1 * args.emb_2D_d2 == args.time_dim)
        ...
        h_out = 2 * self.emb_2D_d1 - self.w_d + 1 #18
        w_out = self.emb_2D_d2 - self.w_d + 11  #18
        self.feat_dim = self.num_out_channels * h_out * w_out
        self.fc = nn.Linear(self.feat_dim, self.entity_dim)

    def forward(self, e1, r, t, kg):
        ...
        T = kg.get_time_embeddings(t).view(-1, 1, self.emb_2D_d1, self.emb_2D_d2)
        E2 = kg.get_all_entity_embeddings()
        # print(E1.size())
        # print(R.size())
        # print(T.size())

        stacked_inputs = torch.cat([E1, torch.cat([R, T], 2)], 2)
    ...
    def forward_fact(self, e1, r, e2, t, kg):
        """
...
class TuckER(nn.Module):
    def __init__(self, args, num_entities):
        ...
        self.relation_dim = args.relation_dim
        self.time_dim = args.time_dim
        self.register_parameter('W', nn.Parameter(torch.tensor(np.random.uniform(-1, 1, (self.relation_dim, self.time_dim, self.entity_dim, self.entity_dim)), dtype=torch.float, device="cuda", requires_grad=True)))
    ...
    def forward(self, e1_idx, r_idx, t_idx, kg):
        ...
        t = kg.get_time_embeddings(t_idx)
        W_mat = torch.mm(r, t, set.W.view(t.size(1)), self.W.view(r.size(1), -1))
    ...
    def forward_fact(self, e1_idx, r_idx, e2_idx, t_idx, kg):
...
class TransE(nn.Module):
    ...
    def forward_train(self, e1, e2, r, t, kg):
        E1 = kg.get_entity_embeddings(e1)
        R = kg.get_relation_embeddings(r)
        T = kg.get_time_embeddings(t)
        E2 = kg.get_entity_embeddings(e2)
        return torch.abs(E1 + R + T - E2)
    ...
    def forward(self, e1, r, t, kg):
        ...
        A = torch.sum((E1 + R + T) * (E1 + R + T), dim=1)
        B = torch.sum(E2 * E2, dim=1)
        AB = torch.mm((E1 + R + T), E2.transpose(1, 0))
    ...
    def forward_fact(self, e1, r, e2, t, kg):
        ...
        R = kg.get_relation_embeddings(r)
        E2 = kg.get_entity_embeddings(e2)
        T = kg.get_relation_embeddings(t)
        return torch.sigmoid(-torch.sqrt(torch.sum((E1 + R + T - E2) * (E1 + R + T - E2), dim=1, keepdim=True)))
...
class PTransE(nn.Module):
    ...
    def forward_train(self, e1, e2, r, t, kg):
        ...
        R = kg.get_relation_embeddings(r)
        E2 = kg.get_entity_embeddings(e2)
        T = kg.get_time_embeddings(e2)
        return torch.abs(E1 + R + T - E2)

    def forward_train_relation(self, e1, e2, r, t, kg, corrupt=False, corrupt_r=None, corrupt_t=None):
        print("^^^")
        batch_size = e1.size()[0]
        relation_weight = Variable(torch.zeros(batch_size, kg.num_relations), requires_grad=False).cuda()
        time_weight = Variable(torch.zeros(batch_size, kg.num_times), requires_grad=False).cuda()
        for i in range(batch_size):
            e1_id, e2_id, r_id, t_id = int(e1[i]), int(e2[i]), int(r[i]), int(t[i])
            path_info = kg.triple2path[(e1_id, e2_id, r_id, t_id)]
            for path in path_info:
                prob = path[-1]
                for relation in path[0]:
                    relation_weight[i][relation] += prob
                for time in path[0]:
                    time_weight[i][time] += prob
        P = torch.mm(relation_weight, kg.get_all_relation_embeddings())
        P2 = torch.mm(time_weight, kg.get_all_time_embeddings())
        if corrupt == False:
            R = kg.get_relation_embeddings(r)
            T = kg.get_time_embeddings(t)
        else:
            ...
            T = kg.get_time_embeddings(t)
        return torch.abs(P + P2 - R - T)

    def forward(self, e1, r, t, kg, path_trace):
        return None

    def forward_fact(self, e1, r, e2, t, kg, path_trace):
        E1 = kg.get_entity_embeddings(e1)
        R = kg.get_relation_embeddings(r)
        E2 = kg.get_entity_embeddings(e2)
        T = kg.get_time_embeddings(t)
...
def get_conve_kg_state_dict(state_dict):
    kg_state_dict = dict()
    for param_name in ['kg.entity_embeddings.weight', 'kg.relation_embeddings.weight', 'kg.time_embeddings.weight']:
