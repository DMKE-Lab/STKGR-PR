...
class EmbeddingBasedMethod(LFramework):
    ...
    def forward_fact(self, examples):
        ...
        for example_id in tqdm(range(0, len(examples), self.batch_size)):
            ...
            pred_score = mdl.forward_fact(e1, r, e2, t, kg)
            pred_scores.append(pred_score[:mini_batch_size])
        return torch.cat(pred_scores)

    def get_corrupt_triple(self, e1, e2, r, t):
        ...
        for i in range(batch_size):
            if decide_num < 25:
                ...
                try:
                    while int(e2[i]) in self.kg.train_objects[rand_e1][int(r[i])][int(t[i])]:
                        rand_e1 = random.randint(0, len(self.kg.entity2id) - 1)
                except:
                    pass
                ...
                try:
                    while rand_e2 in self.kg.train_objects[int(e1[i])][int(r[i])][int(t[i])]:
                        rand_e2 = random.randint(0, len(self.kg.entity2id) - 1)
                except:
                    pass
                ...
                rand_t = random.randint(0, len(self.kg.time2id) - 1)
                try:
                    while int(e2[i]) in self.kg.train_objects[int(e1[i])][rand_r][rand_t]:
                        rand_r = random.randint(0, len(self.kg.relation2id) - 1)
                        rand_t = random.randint(0, len(self.kg.time2id) - 1)
                except:
                    pass
                ...
                t_list.append(rand_t)
        return var_cuda(torch.LongTensor(e1_list), requires_grad=False), var_cuda(torch.LongTensor(e2_list), requires_grad=False), var_cuda(torch.LongTensor(r_list), requires_grad=False), var_cuda(torch.LongTensor(t_list), requires_grad=False)

    def get_corrupt_relation(self, e1, e2, r, t):
        batch_size = e1.size()[0]
        r_list = []
        for i in range(batch_size):
            rand_r = random.randint(0, len(self.kg.relation2id) - 1)
            try:
                while int(e2[i]) in self.kg.train_objects[int(e1[i])][rand_r][t[i]]:
                    rand_r = random.randint(0, len(self.kg.relation2id) - 1)
            except:
                pass
            r_list.append(rand_r)
    ...
    def get_corrupt_time(self, e1, e2, r, t):
        batch_size = e1.size()[0]
        t_list = []
        for i in range(batch_size):
            rand_t = random.randint(0, len(self.kg.time2id) - 1)
            try:
                while int(e2[i]) in self.kg.train_objects[int(e1[i])][r[i]][rand_t]:
                    rand_t = random.randint(0, len(self.kg.time2id) - 1)
            except:
                pass
            t_list.append(rand_t)
        return var_cuda(torch.LongTensor(t_list), requires_grad=False)
    ...
    def loss(self, mini_batch):
        ...
        if self.model == '!TransE':
            e1, e2, r, t = self.format_batch(mini_batch)
            corrupt_e1, corrupt_e2, corrupt_r, corrupt_t = self.get_corrupt_triple(e1, e2, r, t)
            pred_scores = mdl.forward_train(e1, e2, r, t, kg)
            pred_scores = torch.sum(pred_scores, 1)
            corrupt_scores = mdl.forward_train(corrupt_e1, corrupt_e2, corrupt_r, corrupt_t, kg)
            corrupt_scores = torch.sum(corrupt_scores, 1)
            y = Variable(torch.Tensor([-1])).cuda()
            loss = self.loss_fun(pred_scores, corrupt_scores, y)
        else:
            e1, e2, r, t = self.format_batch(mini_batch, num_labels=kg.num_entities)
            e2_label = ((1 - self.label_smoothing_epsilon) * e2) + (1.0 / e2.size(1))
            pred_scores = mdl.forward(e1, r, t, kg)
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        if self.model == 'hypere':
            pred_scores = mdl.forward(e1, r, t, kg, [self.secondary_kg])
        elif self.model == 'triplee':
            pred_scores = mdl.forward(e1, r, t, kg, [self.secondary_kg, self.tertiary_kg])
        else:
            pred_scores = mdl.forward(e1, r, t, kg)
        return pred_scores
    ...
    def export_fuzzy_facts(self):
        ...
        for e1_id in sub_rel:
            for r_id in sub_rel[e1_id]:
                for t_id in sub_rel[e1_id][r_id]:
                    e1_ids.append(e1_id)
                    r_ids.append(r_id)
                    t_ids.append(t_id)
            ...
            for j in range(pred_scores.size(0)):
                for _e2 in range(pred_scores.size(1)):
                    ...
                    if pred_scores[j, _e2] >= self.theta:
                        ...
                        _t = int(t[j])
                        o_f.write('{}\t{}\t{}\t{}\t{}\n'.format(
                            kg.id2entity[_e1], kg.id2entity[_e2], kg.id2relation[_r], kg.id2time[_t], float(pred_scores[j, _e2])))
                        count += 1
                        if count % 1000 == 0:
        ...
        for e2_id in rel_obj:
            for r_id in rel_obj[e2_id]:
                for t_id in rel_obj[e2_id][r_id]:
                    e2_ids.append(e2_id)
                    r_ids.append(r_id)
                    t_ids.append(t_id)
        ...
        for i in range(len(e2_ids)):
            ...
            t = int_fill_var_cuda(e1.size(), t_ids[i])
            e2 = int_fill_var_cuda(e1.size(), e2_ids[i])
            pred_scores = mdl.forward_fact(e1, r, e2, t, kg)
            for j in range(pred_scores.size(1)):
                if pred_scores[j] > self.theta:
                    ...
                    if _e1 in sub_rel and _r in sub_rel[_e1]:
                        continue
                    o_f.write('{}\t{}\t{}\t{}\t{}\n'.format(
                        kg.id2entity[_e1], kg.id2entity[_e2], kg.id2relation[_r], kg.id2time[_t], float(pred_scores[j])))
                    count += 1
                    if count % 1000 == 0:
                        print('{} fuzzy facts exported'.format(count))
