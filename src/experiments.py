...
def train(lf):
    ...
    time_index_path = os.path.join(args.data_dir, 'time2id.txt')
    if lf.model.startswith('point'):
        train_data = data_utils.load_triples(
        train_path, entity_index_path, relation_index_path, time_index_path, group_examples_by_query=args.group_examples_by_query,
    ...
    if 'NELL' in args.data_dir:
        ...
        seen_entities = set()
    dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, time_index_path, seen_entities=seen_entities)
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        to_m_rels, to_1_rels, to_m_tims, to_1_tims, _ = data_utils.get_relations_by_type(args.data_dir, relation_index_path, time_index_path)

        relation_by_types = (to_m_rels, to_1_rels, to_m_tims, to_1_tims)
        ...
        dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, time_index_path, seen_entities=seen_entities)
        pred_scores = lf.forward(dev_data, verbose=False)
        seen_queries = data_utils.get_seen_queries(args.data_dir, entity_index_path, relation_index_path, time_index_path)
        ...
        dev_data = data_utils.load_triples(
            dev_path, entity_index_path, relation_index_path, time_index_path, seen_entities=seen_entities, verbose=False)
        ...
        test_data = data_utils.load_triples(
            test_path, entity_index_path, relation_index_path, time_index_path, seen_entities=seen_entities, verbose=False)
...
def run_ablation_studies(args):
    ...
    dev_data = data_utils.load_triples(
        dev_path, entity_index_path, relation_index_path, time_index_path, seen_entities=seen_entities, verbose=False)
    to_m_rels, to_1_rels, to_M_tims, to_1_tims, (to_m_ratio, to_1_ratio) = data_utils.get_relations_by_type(args.data_dir, relation_index_path, time_index_path)
    relation_by_types = (to_m_rels, to_1_rels)
    to_m_ratio *= 100
    to_1_ratio *= 100
    seen_queries, (seen_ratio, unseen_ratio) = data_utils.get_seen_queries(args.data_dir, entity_index_path, relation_index_path, time_index_path)
    seen_ratio *= 100
    unseen_ratio *= 100
    ...
    for system in systems:
        ...
        pred_scores = lf.forward(dev_data, verbose=False)
        _, _, _, _, mrr = src.eval.hits_and_ranks(dev_data, pred_scores, lf.kg.dev_objects, verbose=True, kg = KnowledgeGraph(args))
        if to_1_ratio == 0:
            to_m_mrr = mrr
            to_1_mrr = -1
        else:
        ...
        _, _, _, _, mrr_full_kg = src.eval.hits_and_ranks(dev_data, pred_scores, lf.kg.all_objects, verbose=True, kg=KnowledgeGraph(args))
        if to_1_ratio == 0:
...
def export_error_cases(lf):
    ...
    time_index_path = os.path.join(args.data_dir, 'time2id.txt')
    dev_path = os.path.join(args.data_dir, 'dev.triples')
    dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, time_index_path)
    lf.load_checkpoint(get_checkpoint_path(args))
    print('Dev set performance:')
    pred_scores = lf.forward(dev_data, verbose=False)
...
def compute_fact_scores(lf):
    ...
    relation_index_path = os.path.join(args.data_dir, 'relation2id.txt')
    time_index_path = os.path.join(args.data_dir, 'time2id.txt')
    train_data = data_utils.load_triples(train_path, entity_index_path, relation_index_path, time_index_path)
    dev_data = data_utils.load_triples(dev_path, entity_index_path, relation_index_path, time_index_path)
    test_data = data_utils.load_triples(test_path, entity_index_path, relation_index_path, time_index_path)
    lf.eval()
    lf.load_checkpoint(get_checkpoint_path(args))
